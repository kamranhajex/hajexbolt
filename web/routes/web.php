...
$startCursor = @$_GET['startCursor'];
$startFrom = 'first';
if($startCursor){
  $startFrom = 'last';
}
$query = '{
  products('.$startFrom.': 20';
if($endCursor){
  $query .= ', after: "' . $endCursor . '"';
}  
if($startCursor){
  $query .= ', before: "' . $startCursor . '"';
}  
$query .=  
  ') {
    edges {
    ...
    pageInfo {
      ...
      hasPreviousPage
        ...
        $ch = curl_init('https://'.$shop.'/admin/api/2021-07/graphql.json');
        ...
        // $next_id = @$response_json->data->products->pageInfo;
        curl_close($ch);
        // dmpe($response_json->data);
        exit;
        $max_per_page = 20;
        // $since_ids = [];
        // if(!@$_GET['since_id']){
        ...
        $products = [];
        $since_id = (@$_GET['since_id']) ? @$_GET['since_id'] : 0;
        $ch = curl_init();
        $url = 'https://'.$shop.'/admin/api/2021-10/products.json?limit='.$max_per_page;
        if($since_id){
        ...
        if (curl_errno($ch)) {
          ...
          // echo '</pre>';exit;
          $next_id = @$list[count($list) - 1]->id;
          $previous_id = @$list[0]->id;
